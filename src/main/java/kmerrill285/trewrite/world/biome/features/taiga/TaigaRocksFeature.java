...
public class TaigaRocksFeature extends Feature<NoFeatureConfig> {
	
		
	
	   public TaigaRocksFeature(Function<Dynamic<?>, ? extends NoFeatureConfig> configFactoryIn) {
		   ...
		   for(int i = 0; i < rand.nextInt(5) + 5; ++i) {
	         ...
	         if (worldIn.getBlockState(blockpos) == BlocksT.PODZOL.getDefaultState() || worldIn.getBlockState(blockpos) == BlocksT.GRASS_BLOCK.getDefaultState()) {
	        	 if (rand.nextInt(10) <= 4)
	        		 ...
	        		 worldIn.setBlockState(pos.up(), BlocksT.STONE_BLOCK.getDefaultState(), 2);
	        		 worldIn.setBlockState(pos.up().up(), BlocksT.STONE_BLOCK.getDefaultState(), 2);
	        	 }
		 		...
		 		for (int y = 0; y < 64; y++) {
		 			for (int z = 0; z < 64; z++) {
			 			if (stuff[x][y][z] == Color.BLACK) {
			 				...
			 				worldIn.setBlockState(pos2, BlocksT.STONE_BLOCK.getDefaultState(), 2);
			 			}
